...
[TeraiD Pay API](https://github.com/terao06/TeraiD-pay-api)よりAPI用ソースをcloneしてください。  
...
| `NEXT_PUBLIC_RPC_URL_AVALANCHE_FUJI` | 推奨 | Avalanche Fuji に接続する RPC URL です。Fuji のウォレット情報を扱う場合に使用します。 |
| `NEXT_PUBLIC_RPC_URL_POLYGON_AMOY` | 推奨 | Polygon Amoy に接続する RPC URL です。Amoy のウォレット情報を扱う場合に使用します。例: `https://rpc-amoy.polygon.technology` |
| `NEXT_PUBLIC_JPYC_TOKEN_ADDRESS` | 条件付き | Ethereum Sepolia / Avalanche Fuji / Polygon Amoy 上の JPYC トークンコントラクトアドレスです。対象ウォレットが Sepolia、Fuji、Amoy の JPYC の場合に残高表示で使用します。 |
...
JPYC のトークンコントラクトアドレスは、設定するネットワークごとに異なります。必ず対象ネットワークに対応したアドレスを確認してください。

確認方法の優先順位は次のとおりです。

1. JPYC の公式ドキュメント、公式サイト、または公式案内で対象ネットワークのコントラクトアドレスを確認します。
2. 公式情報に記載されているアドレスを、Etherscan や Polygonscan などのブロックエクスプローラーで検索します。
3. ブロックエクスプローラー上で、トークン名、シンボル、ネットワーク、コントラクトアドレスが一致していることを確認します。
4. 確認したアドレスを `.env.local` の該当する `NEXT_PUBLIC_JPYC_TOKEN_ADDRESS_*` に設定します。

設定先の対応は次のとおりです。

| ネットワーク | 設定する環境変数 |
| --- | --- |
| Ethereum Mainnet | `NEXT_PUBLIC_JPYC_TOKEN_ADDRESS_ETHEREUM_MAINNET` |
| Ethereum Sepolia | `NEXT_PUBLIC_JPYC_TOKEN_ADDRESS` |
| Avalanche Fuji | `NEXT_PUBLIC_JPYC_TOKEN_ADDRESS` |
| Polygon Mainnet | `NEXT_PUBLIC_JPYC_TOKEN_ADDRESS_POLYGON_MAINNET` |
| Polygon Amoy | `NEXT_PUBLIC_JPYC_TOKEN_ADDRESS` |

注意点:

- Mainnet と Testnet ではコントラクトアドレスが異なります。
- Ethereum と Polygon でもコントラクトアドレスは異なります。
- 検索結果、SNS、第三者の記事だけを根拠に設定しないでください。
- ブロックエクスプローラーで同名・同シンボルの別トークンが見つかる場合があります。必ず公式情報と照合してください。
...
## 画面イメージ
### ウォレット登録画面
![ウォレット登録ページ](images/wallet_register.png)

### ウォレット詳細画面
![ウォレット詳細ページ](images/wallet_detail.png)
